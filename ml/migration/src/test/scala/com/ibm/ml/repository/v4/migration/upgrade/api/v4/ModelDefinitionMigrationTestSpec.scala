...
trait ModelDefinitionMigrationTestSpec extends MLRepoMigrationTestBase with GivenWhenThen{

  "Testing modelbDefinition migration" when {
    "MDEF_MIG_1: v4beta modelDefinition should be migrated to v4ga repository" in {
      Given("A v4Beta modelDefinition")
      val modelDefinitionId = createV4BetaModelDefinition(spaceId = spaceId, projectId = projectId).metadata.assetId
      val (newSpaceId, newProjectId) = createNewContainer()

      When("Migration service is invoked for the v4Beta modelDefinition")
      val migrationResult = Await.result((for{
        migrationDoc <- startMigration(modelDefinitionIds = Some(Seq(modelDefinitionId.get)), newSpaceId = newSpaceId, newProjectId = newProjectId)
        _ <- futureSleep(200)
      ...
      And("v4Beta modelDefinition is migrated to the new repository")
      val maybeModelJobDef = migrationResult.results.get.successful.find(_.newAssetType.equals("model_definitions"))
      assert(maybeModelJobDef.isDefined)
      //GET on the V4GA asset should return 200
      val v4gaModelJobDef = client.modelDefinitions.getById(id = maybeModelJobDef.get.newId, expectedStatus = Seq(200), spaceId = newSpaceId, projectId = newProjectId)

      And("v4ga asset contains 'migrated_from' custom field with fields 'asset_id', 'asset_type', 'instance_id'")
      assert(getMigratedFrom(v4gaModelJobDef._1.entity.custom)._1 == modelDefinitionId.get)
    }
  }
