...
object TestCloudantClient extends StrictLogging {
...
case class TestCloudantClient(dbUrl: String,
                                dbName: String,
                                username: String,
                                password: String,
  ...
  designDocPath.map { path => {
    Try {
      ...
      case Failure(e) =>
        ExceptionLogger.log(Some("Failed to update design doc for Cloudant DB $dbName"), e, None, callStack = true)
        throw e
    }
  }
  }
  ...
  def save(payload: JsValue): Future[JsValue] = Future {
    TestCloudantClient.handleException {
