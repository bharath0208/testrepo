...
import com.ibm.ml.repository.v4.utils.{FailedToConvertToRequest, FailedToCreateNewAsset, FailedToGetAsset, FailedToGetDependencies, FailedToMigrateAsset, FailedToMigrateDependencies, ServiceException, logPrint, seqFutures}
...
abstract class AbstractMigrationRoute[OldResource, NewRequest](mj: MigrationJob)(implicit system: ActorSystem,
  ...
  def createMigratedFrom(oldAssetId: String, doc: MigrationDoc): MigratedFrom = {
    MigratedFrom(
      projectId = doc.oldProjectId,
      spaceId = doc.oldSpaceId,
      assetId = oldAssetId
    )
  }

  def addMigratedFrom(custom: Option[JsObject], oldAssetId: String, doc: MigrationDoc): Option[JsObject] = {
    val MIGRATED_FROM: String = "migrated_from"

    Try(createMigratedFrom(oldAssetId, doc).toJson) match {
      case Success(from) =>
        custom match {
          case Some(jsObject) =>
  ...
  private def shouldSkip(oldId: String, migratedAssets: Seq[MigrationSuccessfulResult], doc: MigrationDoc): Future[Option[MigrationSuccessfulResult]] = {
    if (doc.shouldSkipMigratedAssets) {
      seqFutures(migratedAssets.filter(_.oldId == oldId).toVector) { result =>
        for {
  ...
  private def migrateAsset(oldId: String, migratedAssets: Seq[MigrationSuccessfulResult], doc: MigrationDoc): Future[MigrationDoc] = {
    for {
      ...
      (oldResource, newId, consoleMsgs) <- getOldAsset(oldId, doc: MigrationDoc)
  ...
  protected def getNameAndLastModified(oldResource: Option[OldResource]): (Option[String], Option[Date]) = {
  ...
  private def handleMigrationFailure(oldId: String,
    ...
    for {
      ...
      results: MigrationResults = latestDoc.results.getOrElse(MigrationResults())
  ...
  private def getOldAsset(id: String, doc: MigrationDoc): Future[(OldResource, Option[String], Seq[String])] = {

    (for {
      (resource, newId, consoleMsgs) <- getOldAssetAction(id, doc)
    } yield {
  ...
  protected def getOldAssetAction(id: String, doc: MigrationDoc): Future[(OldResource, Option[String], Seq[String])]
