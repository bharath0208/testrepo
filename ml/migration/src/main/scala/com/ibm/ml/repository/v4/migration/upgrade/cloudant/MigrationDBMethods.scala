...
case class MigrationDBMethods(client: SimpleCloudantClient) extends MigrationConstant with StrictLogging {
  ...
  private def convertToMigrationResource(dbResource: JsValue): MigrationResource = {
    Try {
      ...
      MigrationResource(
        ...
        migrationDoc.oldSpaceId,
        migrationDoc.oldProjectId,
        migrationDoc.newSpaceId,
        migrationDoc.newProjectId,
    ...
    for {
      dbPayload <- Future.fromTry {
        Try {
          ...
          MigrationDoc(userId = Some(identity.subject.id),
            status = Pending,
            ...
            newSpaceId = request.newSpaceId,
            newProjectId = request.newProjectId,
            oldSpaceId = request.oldSpaceId,
            oldProjectId = request.oldProjectId,
  ...
  def getAllMigrationDocsByStatus(Status: MigrationStatus)
                                 (implicit ec: ExecutionContext): Future[Vector[MigrationDoc]] = {
    for {
      json <- client.view(LIST_JOB_BY_STATUS_VIEW, Some(List(Status.name)))
    } yield {
      json.convertTo[Vector[MigrationDoc]]
    }
  }
  ...
  def getAllMigrationDocs(implicit ec: ExecutionContext): Future[Vector[MigrationDoc]] = {
    for {
      json <- client.view(GET_ALL_JOBS_VIEW, None)
    } yield {
      json.convertTo[Vector[MigrationDoc]]
    }
  }
