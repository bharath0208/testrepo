...
trait PipelineConverter extends ConversionHelper {
            ...
            nodes = pipelineDef.nodes.map(node =>
              node.copy(
                parameters = node.parameters.map { nodeParams =>
                  ...
                  val trainingLib = nodeParams.get(FIELD_TRAINING_LIB_HREF)
                  val modelDef = nodeParams.get(FIELD_MODEL_DEFINITION)

                  if (trainingLib.isDefined) {
                    trainingLib.map(trainingLibRef => {
                      val trainingLibRefStr = trainingLibRef.asInstanceOf[JsString].value
                      val assetType = if (trainingLibRefStr.contains("/v4/libraries/")) TRAINING_LIBS_TYPE else MODEL_DEFINITIONS_TYPE
                      val modelDefId = findResultId(hrefToId(trainingLibRefStr), assetType, results, mappings)
                  ...
                  } else if (modelDef.isDefined) {
                    modelDef.flatMap(md => {

                      Try {
                        val oldId = md.asInstanceOf[JsString].value
                        val modelDefId = findResultId(oldId, MODEL_DEFINITIONS_TYPE, results, mappings)
                        import com.ibm.analytics.wml.api.v4ga.common.CommonJsonFormat._
                        Some(nodeParams - FIELD_MODEL_DEFINITION + (FIELD_MODEL_DEFINITION -> ResourceRef(id = modelDefId).toJson))
                      }.getOrElse(None)
  ...
  protected def convertWMLData(wmlData: WmlData,
    ...
    (wmlData.runtimeSpecV4, wmlData.runtimeSpec) match {
      ...
      case (None, Some(runtimeSpec)) =>
        ...
        ((wmlDataMap ++ fields).toJson.convertTo[WmlData], consoleMsgs)
      case _ =>
        // here we need to make sure software spec in wmlDataMap will override the default value
        ((addAutoAIcSoftwareSpec(runtimeName, softwareSpecs) ++ wmlDataMap).toJson.convertTo[WmlData], Seq.empty)
    }
  }
  ...
  protected def addAutoAIcSoftwareSpec(runtimeName: String, softwareSpecs: SoftwareSpecifications): Map[String, JsValue] = {
    runtimeName match {
      case AUTO_AI_KB =>
        val KBSoftwareSpec = getBaseSoftwareSpecByName(softwareSpecs, AUTO_AI_KB_SW_SPEC_NAME)
        KBSoftwareSpec
