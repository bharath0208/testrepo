...
import com.ibm.ml.repository.v4.migration.converters.FunctionConverter
...
case class FunctionMigrationRoute(mj: MigrationJob)(implicit system: ActorSystem, identity: Identity)
  ...
  override protected def getOldAssetAction(id: String): Future[(FunctionResource, Option[String], Seq[String])] = {
    for {
      oldFunction <- mj.sc.v4BetaRepositoryClient.functions.get(identity = identity, id = id)
  ...
  override protected def convertToRequestAction(oldResource: FunctionResource, dependencies: MigrationIds, doc: MigrationDoc): Future[(FunctionEntityRequest, Seq[String])] = {
    ...
    val defaultSoftwareSpec = getBaseSoftwareSpecByName(mj.softwareSpecs, DEFAULT_FUNCTION_SOFTWARE_SPEC)
  ...
  override protected def createNewAssetAction(request: FunctionEntityRequest, oldResource: FunctionResource, doc: MigrationDoc): Future[String] = {
    ...
    for {
      (resource, _) <- mj.sc.mlRepositoryClient.functions.create(
        ...
        entity = request.copy(custom = addMigratedFrom(request.custom, oldResource.metadata.id))
      )
      _ <- {
        (for {
          source <- mj.sc.v4BetaRepositoryClient.functions.download(identity, oldId)
