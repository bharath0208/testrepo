...
import com.ibm.ml.repository.v4.migration.models.{MigrationDoc, MigrationIds}
import spray.json._
...
case class TrainingLibMigrationRoute(mj: MigrationJob)(implicit system: ActorSystem, identity: Identity)
  ...
  override protected def getOldAssetAction(id: String): Future[(LibraryResource, Option[String], Seq[String])] = {
    for {
      resource <- mj.sc.v4BetaRepositoryClient.library.get(identity, id)
    } yield {
  ...
  override protected def createNewAssetAction(request: ModelDefinitionEntityRequest, oldResource: LibraryResource, doc: MigrationDoc): Future[String] = {
    ...
    for {
      (resource, _) <- mj.sc.mlRepositoryClient.model_definitions.create(identity, ML_REPO_VERSION, request.copy(custom = addMigratedFrom(request.custom, oldResource.metadata.id)))

      _ <- {
        (for {
          source <- mj.sc.v4BetaRepositoryClient.library.download(identity, oldId)
