...
import com.ibm.analytics.wml.api.v4.common.{HyperReference, ModelDefinitionRef, TrainingLibRef}
...
trait ExperimentConverter extends ConversionHelper {
  ...
  def convertTrainingReference(trainingReference: v4.experiments.TrainingReference,
    ...
    val (newModelDef, consoleMsgs) = convertModelDef(trainingReference.trainingLib, trainingReference.modelDefinition, hardwareSpecs, results, mappings, oldAssetId)
    import v4ga.experiments.ExperimentJsonFormat._
  ...
  protected def convertModelDef(trainingLib: Option[TrainingLibRef],
                                oldModelDef: Option[ModelDefinitionRef],
                                hardwareSpecs: HardwareSpecificationResources,
                                results: MigrationResults,
                                mappings: Option[Map[String, String]],
                                oldAssetId: String): (Map[String, JsValue], Seq[String]) = {
    if (trainingLib.isDefined)
      convertTrainingLibRefToModelDef(trainingLib, hardwareSpecs, results, mappings, oldAssetId)
    else if (oldModelDef.isDefined)
      convertModelDefToModelDef(oldModelDef, results, mappings, oldAssetId)
    else
      (Map.empty, Seq.empty)
  }
  ...
  protected def convertModelDefToModelDef(oldModelDef: Option[ModelDefinitionRef],
                                          results: MigrationResults,
                                          mappings: Option[Map[String, String]],
                                          oldAssetId: String): (Map[String, JsValue], Seq[String]) = {
    import com.ibm.analytics.wml.api.v4.common.CommonJsonFormat._
    oldModelDef.map { md =>
      val tlMap = md.toJson.asJsObject.fields - FIELD_ID
      val newId = jsonMap(FIELD_ID, findResultId(md.id, MODEL_DEFINITIONS_TYPE, results, mappings).toJson)

      val newModelDefinitionRef = (tlMap ++ newId).toJson
      (jsonMap(FIELD_MODEL_DEFINITION, newModelDefinitionRef), Seq.empty)
    }.getOrElse(Map.empty, Seq.empty)
  }
