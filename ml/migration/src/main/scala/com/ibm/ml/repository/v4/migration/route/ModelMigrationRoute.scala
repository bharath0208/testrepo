...
import com.ibm.ml.repository.v4.migration.converters.ModelConverter
...
case class ModelMigrationRoute(mj: MigrationJob)(implicit system: ActorSystem, identity: Identity)
  ...
  override protected def getOldAssetAction(id: String): Future[(ModelResource, Option[String], Seq[String])] = {
    for {
      resource <- mj.sc.v4BetaRepositoryClient.models.get(identity, id)
    } yield {
  ...
  override protected def convertToRequestAction(oldResource: ModelResource, dependencies: MigrationIds, doc: MigrationDoc): Future[(ModelEntityRequest, Seq[String])] = {
    ...
    val oldEntity = oldResource.entity.toJson.asJsObject.fields - FIELD_TAGS - FIELD_SPACE - FIELD_NAME -
      FIELD_PROJECT - FIELD_SPACE_ID - FIELD_PROJECT_ID - FIELD_PIPELINE - FIELD_TRAINING_LIB -
    ...
    val defaultSoftwareSpec = getBaseSoftwareSpecByName(mj.softwareSpecs, DEFAULT_MODEL_SOFTWARE_SPEC)
    ...
    val trainingLib = convertResourceRef(
      entity.trainingLib,
      TRAINING_LIBS_TYPE,
    ...
    val request = (oldEntity ++ newName ++ newIds ++ newTags ++ softwareSpecRef ++ pipeline ++ trainingLib ++ trainingDataReferences).toJson.convertTo[ModelEntityRequest]
  ...
  override protected def createNewAssetAction(request: ModelEntityRequest, oldResource: ModelResource, doc: MigrationDoc): Future[String] = {
    ...
    val projectId = doc.projectId
    val spaceId = doc.spaceId


    for {
      (resource, _) <- mj.sc.mlRepositoryClient.models.create(identity, ML_REPO_VERSION, request.copy(custom = addMigratedFrom(request.custom, oldResource.metadata.id)))
      _ <- {
        ...
        val uploadList = if (entity.modelType.contains(MODEL_TYPE_HYBRID)) {
          for {
            source <- mj.sc.v4BetaRepositoryClient.models.download(identity, oldId, artifact = Some(ARTIFACT_PIPELINE_MODEL))
  ...
  private def uploadModelSingleContent(oldId: String,
    ...
    (for {
      source <- mj.sc.v4BetaRepositoryClient.models.download(identity, oldId, artifact = artifact, format = oldFormat)
      _ <- mj.sc.mlRepositoryClient.models.upload(identity,
        ML_REPO_VERSION,
        newId,
        None,
