...
package com.ibm.ml.repository.v4.migration.upgrade.converters
...
import com.ibm.ml.repository.v4.migration.upgrade.models.MigrationResults
...
import com.ibm.ml.repository.v4.utils.{FailedToMigrateDependencies, ServiceException, isPublicCloud}
...
trait ConversionHelper extends MigrationConstant with StrictLogging {
  ...
  protected def findResultId(oldId: String, oldAssetType: String, results: MigrationResults, mapping: Option[Map[String, String]] = None): String = {
    ...
    results.skipped.find(result => result.oldId == oldId && result.oldAssetType == oldAssetType).foreach { result =>
      return result.newId
    }
  ...
  protected def getBaseSoftwareSpecByName(softwareSpecs: SoftwareSpecifications, softwareSpecName: String, overrideException: Option[Throwable] = None): Map[String, JsValue] = {
    softwareSpecs.resources.find(swSpec => swSpec.metadata.name.getOrElse("") == softwareSpecName) match {
      case Some(softwareSpec) =>
        import com.ibm.analytics.wml.api.v4ga.common.CommonJsonFormat._
        upgradeSoftwareSpec(softwareSpecs, Some(softwareSpec.metadata.assetId), Some(softwareSpecName))
      case _ =>
  ...
  protected def getSoftwareSpecByid(softwareSpecs: SoftwareSpecifications, id: String): Map[String, JsValue] = {
    softwareSpecs.resources.find(swSpec => swSpec.metadata.assetId == id) match {
      case Some(softwareSpec) =>
        import com.ibm.analytics.wml.api.v4ga.common.CommonJsonFormat._
        jsonMap(FIELD_SOFTWARE_SPEC, SoftwareSpecRef(id = Some(softwareSpec.metadata.assetId), name = softwareSpec.metadata.name).toJson)
      case _ =>
        throw ServiceException(StatusCodes.BadRequest,
          FailedToMigrateDependencies(s"Could not find software specification match the id '$id'"))
    }
  }

  protected def upgradeSoftwareSpec(softwareSpecs: SoftwareSpecifications, oldId: Option[String], oldName: Option[String]): Map[String, JsValue] = {
    val softwareSpecUpgradeMap = softwareSpecUpgradeCommonMap ++ (if(isPublicCloud) softwareSpecUpgradePublicMap else softwareSpecUpgradePrivateMap)
    import com.ibm.analytics.wml.api.v4ga.common.CommonJsonFormat._
    oldId.map { id =>
      softwareSpecUpgradeMap.get(id).map(newSoftwareSpecId => {
        getSoftwareSpecByid(softwareSpecs, newSoftwareSpecId)
      }).getOrElse(jsonMap(FIELD_SOFTWARE_SPEC, SoftwareSpecRef(id = oldId).toJson))
    }.getOrElse(jsonMap(FIELD_SOFTWARE_SPEC, SoftwareSpecRef(id = oldId, name = oldName).toJson))
  }
