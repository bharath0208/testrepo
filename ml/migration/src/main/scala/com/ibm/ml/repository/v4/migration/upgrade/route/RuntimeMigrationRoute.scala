...
case class RuntimeMigrationRoute(mj: MigrationJob)(implicit system: ActorSystem, identity: Identity)
  ...
  override protected def getOldAssetAction(id: String, doc: MigrationDoc): Future[(RuntimeResource, Option[String], Seq[String])] = {
    for {
      runtime <- mj.sc.v4BetaRepositoryClient.runtimes.get(identity = identity, id = id, spaceId = doc.oldSpaceId, projectId = doc.oldProjectId)
    } yield {
